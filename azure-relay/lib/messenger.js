...
class AwsMessenger extends Messenger {
  ...
  isRegistrationComplete() {
    return process.env.AWS_PRIVATE_KEY && process.env.AWS_CERT;
  }
...
class GcpMessenger extends Messenger {
  ...
  async connect() {
    ...
    let params = Object.assign(
      { password: this.createJwt(process.env.GCP_PROJECT_ID, 'ES256') },
      this.mqttParams
    );
    ...
    }, (process.env.GCP_TOKEN_LIFETIME - process.env.GCP_RENEWAL_START) * 60 * 1000);
    console.log('Connected to IoT Core messaging');
  }
  ...
  publish(topic, message) {
    //console.log(`Messenger pub: ${message.toString()}`)
    if (this.connected) {
      this.mqtt.publish('events', message);
    } else {
